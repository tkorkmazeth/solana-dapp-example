...
export const UploadMetadata: FC = ({}) => {
  ...
  const initializeIrys = async () => {
    if (!provider) {
      notify({ type: "error", message: "Provider not connected." });
      return;
    }
    ...
    try {
      ...
      notify({ type: "error", message: `Irys SDK error: ${err.message}` });
      return;
    }

    notify({ type: "success", message: `Connected to ${selected.network}` });
    console.log("irys", irys);

    setAddress(irys.address);
  ...
  const uploadImage = async () => {
    ...
    try {
      const price = await irysInstance.getPrice(imageFile.length);
      ...
      const imageResult = await irysInstance.upload(imageFile, {
        headers: { "Content-Type": "image/png" },
      });

      const arweaveImageUrl = `https://arweave.net/${imageResult.id}?ext=png`;

      if (arweaveImageUrl) {
        setImageUrl(arweaveImageUrl);
      }
    } catch (err) {
      notify({ type: "error", message: `Image Upload Error: ${err.message}` });
    }
  };

  const uploadMetadata = async () => {
    try {
      const price = await irysInstance.getPrice("solana", metadata.length);
      const balance = await irysInstance.getBalance();
      ...
      const metadataResult = await irysInstance.upload(metadata, {
        headers: { "Content-Type": "application/json" },
      });

      const arweaveMetadataUrl = `https://arweave.net/${metadataResult.id}`;

      setMetadataUrl(arweaveMetadataUrl);
    } catch (err) {
      notify({
        type: "error",
        message: `Metadata Upload Error: ${err.message}`,
      });
    }
  };
          ...
          <div className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1">
            <div className="px-4 py-5 bg-white space-y-6 sm:p-6">
              <button
                ...
                onClick={async () => await initializeIrys()}
              >
                Connect
              </button>
            </div>
          ...
          <div className="md:col-span-1">
            <div className="px-4 sm:px-0">
              ...
              <p className="mt-1 text-sm text-gray-600">
                ...
                <code className="text-purple-500 bg-purple-100">uri</code>{" "}
